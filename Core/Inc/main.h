/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define POWER_Pin GPIO_PIN_0
#define POWER_GPIO_Port GPIOA
#define BATTERY_TEMP_Pin GPIO_PIN_1
#define BATTERY_TEMP_GPIO_Port GPIOA
#define SD_CD_Pin GPIO_PIN_4
#define SD_CD_GPIO_Port GPIOA
#define SPI1_CS_Pin GPIO_PIN_0
#define SPI1_CS_GPIO_Port GPIOB
#define USER_LED_Pin GPIO_PIN_1
#define USER_LED_GPIO_Port GPIOB
#define LCD_RESET_Pin GPIO_PIN_2
#define LCD_RESET_GPIO_Port GPIOB
#define LCD_DC_Pin GPIO_PIN_10
#define LCD_DC_GPIO_Port GPIOB
#define LCD_BLK_Pin GPIO_PIN_14
#define LCD_BLK_GPIO_Port GPIOB
#define STDBY_Pin GPIO_PIN_9
#define STDBY_GPIO_Port GPIOA
#define CHARGE_Pin GPIO_PIN_10
#define CHARGE_GPIO_Port GPIOA
#define SD_CS_Pin GPIO_PIN_15
#define SD_CS_GPIO_Port GPIOA
#define SD_SCK_Pin GPIO_PIN_3
#define SD_SCK_GPIO_Port GPIOB
#define SD_MISO_Pin GPIO_PIN_4
#define SD_MISO_GPIO_Port GPIOB
#define SD_MOSI_Pin GPIO_PIN_5
#define SD_MOSI_GPIO_Port GPIOB
#define RETURN_KEY_Pin GPIO_PIN_6
#define RETURN_KEY_GPIO_Port GPIOB
#define RETURN_KEY_EXTI_IRQn EXTI9_5_IRQn
#define UP_KEY_Pin GPIO_PIN_7
#define UP_KEY_GPIO_Port GPIOB
#define UP_KEY_EXTI_IRQn EXTI9_5_IRQn
#define DOWN_KEY_Pin GPIO_PIN_8
#define DOWN_KEY_GPIO_Port GPIOB
#define DOWN_KEY_EXTI_IRQn EXTI9_5_IRQn
#define ENTER_KEY_Pin GPIO_PIN_9
#define ENTER_KEY_GPIO_Port GPIOB
#define ENTER_KEY_EXTI_IRQn EXTI9_5_IRQn

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
